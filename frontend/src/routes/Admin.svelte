<script>
  ...
  let currentTab = $state('users'); // 'users', 'organizations', 'teams', 'boards'
  let users = $state([]);
      ...
      <aside class="sidebar">
        <nav>
          ...
          <button
            ...
            onclick={() => currentTab = 'organizations'}
            disabled
          >
            Organizations
          </button>
          ...
          <button
            class:active={currentTab === 'boards'}
...
<style>
  .loading, .not-admin, .empty-state, .coming-soon {
    display: flex;
