<script>
  ...
  async function handleShare() {
    if (selectedTeamId === board?.shared_team_id) {
      onClose();
      return;
    }
    ...
    {#snippet children()}
      ...
      <div class="modal-actions">
        ...
        <button type="button" class="create-btn" onclick={handleShare} disabled={loading}>
          {loading ? 'Saving...' : (selectedTeamId === board?.shared_team_id ? 'Close' : 'Share')}
        </button>
      </div>
    {/snippet}
  </Modal>
